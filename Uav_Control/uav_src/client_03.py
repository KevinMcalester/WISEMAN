...
class DroneClient:
    def __init__(self):
        try:
            ...
            # For now, connecting by IP while cert is for localhost.
    ...
    def send_framed_message(self, sock, msg_type: str, payload_bytes: bytes) -> bool:
        ...
        try:
            ...
            if msg_type == "video" and len(payload_bytes) > self.max_frame_size:
                raise ValueError(f"Video frame too large: {len(payload_bytes)} bytes")

            sig = self.sign_bytes(type_bytes + payload_bytes)

            type_len_field = struct.pack("!B", len(type_bytes))
            payload_len_field = struct.pack("!Q", len(payload_bytes))

            packet = type_len_field + type_bytes + payload_len_field + payload_bytes + sig
    ...
    def run_video_mode(self, sock) -> None:
        ...
        try:
            ...
            self.log("Starting RealSense pipeline...")
            ...
            self.log("RealSense started successfully. Beginning frame transmission...")
            ...
            while True:
                ...
                if not color_frame:
                    self.log("Failed to get color frame from RealSense.")
                    continue
                ...
                if len(frame_bytes) > self.max_frame_size:
                    self.log(f"Encoded frame too large: {len(frame_bytes)} bytes")
                    continue
                ...
                if frame_counter % 30 == 0:
                    self.log(f"Frame #{frame_counter} sent ({len(frame_bytes)} bytes)")

                time.sleep(self.send_delay)

        except Exception as err:
            self.log("Unexpected error during RealSense video mode.")
            ...
            if pipeline is not None:
                try:
                    ...
                    pass
                self.log("RealSense pipeline stopped.")
...
def main():
    client = DroneClient()
    MODE = "telemetry"   # change to "video" for feed capture
    client.run(MODE)
